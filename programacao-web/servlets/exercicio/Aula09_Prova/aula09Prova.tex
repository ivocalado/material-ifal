...
\item Mostre como modificar a aplicação da Seção 2, de forma que o
formulário também receba os campos endereço e telefone. Mostre todos
os locais que o código será alterado.

\item Explique o que significa as tags \emph{$<$servlet$>$}, \emph{$<$servlet-name$>$}, \emph{$<$servlet-class$>$,
$<$servlet-mapping$>$}, \emph{$<$url-pattern$>$} utilizados na
configuração do arquivo \emph{web.xml}.

\item Mostre como se deve utilizar variáveis de seção e cookies.
Explique quais as diferenças.

\item No momento da implementação de um Servlet, qual a diferença
entre implementar um método \emph{doPost} e \emph{doGet}.

\item Implemente um Servlet que receba as requisições do formulário
existente na página \emph{index.html} que foi alterada na questão 1.
Em seguida, explique como substituir o servlet
\emph{ServletInserirSessao} por esse Servlet, ou seja, o que deve
ser alterado na aplicação da Seção 2?
