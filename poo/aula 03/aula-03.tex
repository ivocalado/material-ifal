...
\begin{frame}{Um pouco mais sobre métodos set e get: JavaBeans}
 \begin{itemize}
  \item A utilização dos métodos set e get é importante visto que diversas bibliotecas de Java levam em consideração essa padronização
 \end{itemize}
 
 \begin{block}{O que seriam JavaBean?}
  Trata-se de uma especificação de software definida pela Sun/Oracle para criação de componentes reutilizáveis. Em termos práticos, um \textbf{bean} é uma classe Java com as seguintes propriedades:
  \begin{itemize}
   \item Possuir construtor \textit{default} (isto é, sem argumentos)
   \item Possuir métodos get e set para os atributos da classe (apenas dos que necessitarem ser acessados). 
  \end{itemize}
