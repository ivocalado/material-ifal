...
\begin{itemize}
 \item A divisão por zero irá disparar uma \textbf{exceção} que deve ser tratada
\end{itemize}
...
\end{figure}
\begin{itemize}
 \item Quando uma exceção é lançada ela começa a interromper todos os métodos da pilha de chamadas até ser \textbf{capturada} ou então até a aplicação ser \textbf{encerrada}
\end{itemize}
...
\begin{frame}[fragile,allowframebreaks]{Capturando uma exceção}
 Para capturarmos uma exceção utilizamos a cláusula \textbf{try-catch}
 \begin{lstlisting}
public class C2 {
...
>fim método main
 ...
 \end{lstlisting}
\begin{itemize}
 \item Vamos agora testar o processo a saída se movermos o bloco try-catch para algum outro método da pilha de chamada. Exemplo:
